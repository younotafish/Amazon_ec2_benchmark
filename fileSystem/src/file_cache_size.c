...
void done_sequential()
{
	...
	if (count)
	{
		...
		printf("\n the count: %d timeout: %d", count, getTimeout());
		printf(".\n[*] Results: %.0f MB/second, %.7f ms access time per block\n",
...
int main(int argc, char **argv)
{
	...
	if (argc != 5)
	{
		// file_cache_size file_name numberblocks out_put_file time_out
		// printf("Usage: file_cache_size  <raw disk device> <fake_file_size_in_mb>\n");
		printf("Usage: file_cache_size file_name numberblocks out_put_file time_out\n");
		exit(EXIT_SUCCESS);
	}
